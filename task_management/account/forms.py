...
class UserRegisterForm(forms.Form):
    username = forms.CharField( widget=forms.TextInput(attrs={"class": "specialscolor"}), max_length = 150,
        help_text = ("Required max 50 & min 7 letter(using both uppercase and lowercase) and digits,acually  with no spaces or special characters &,=,-,',+,,, <,>, or ."), 
    ...
    email=forms.EmailField(widget=forms.EmailInput(attrs={"class": "specialscolor"}), help_text="Please enter a valid email address.",)
    # validators = [UnicodeUsernameValidator()],
    ...
    def clean_username(self):
        ...
        if user:
            self.add_error("username","this username is alreadly exists")
        return username
